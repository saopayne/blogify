...
import nltk
from nltk.corpus import stopwords  # stopwords add no value to content

import re  # regular expression to match content (lowercase and remove numbers)
import string
import pickle  # for storing the trained model
...
train = pd.read_csv("blog-data.csv", header=0, delimiter=",", quoting=2)

nltk.download()

# check shape
print('\nTrain for analysis shape[row,column]: {}'.format(train.shape))
print('\nList of categories to be considered: {}'.format(train.columns.values))
